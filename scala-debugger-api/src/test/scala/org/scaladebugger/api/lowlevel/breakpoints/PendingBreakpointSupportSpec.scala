  ...
  describe("PendingBreakpointSupport") {
    describe("#processAllPendingBreakpointRequests") {
      it("should process all pending breakpoints") {
        ...
        val expected = Seq(
          BreakpointRequestInfo("", true, false, testFileName, testLineNumber),
          BreakpointRequestInfo("", true, false, testFileName + 1, testLineNumber),
          BreakpointRequestInfo("", true, false, testFileName, testLineNumber + 1)
        )
    ...
    describe("#processPendingBreakpointRequestsForFile") {
      it("should process pending breakpoints for the specified file") {
        val expected = Seq(
          BreakpointRequestInfo("", true, false, "file1", 1),
          BreakpointRequestInfo("", true, false, "file1", 999)
        )
        val actions = (expected :+ BreakpointRequestInfo("", true, false, "file2", 1))
    ...
    describe("#pendingBreakpointRequests") {
      it("should return a collection of all pending breakpoints") {
        val expected = Seq(
          BreakpointRequestInfo("", true, false, "file1", 1),
          BreakpointRequestInfo("", true, false, "file1", 999),
          BreakpointRequestInfo("", true, false, "file2", 1)
        )
    ...
    describe("#createBreakpointRequestWithId") {
      ...
      it("should add a pending breakpoint if exception thrown") {
        ...
        (mockPendingActionManager.addPendingActionWithId _).expects(
          TestRequestId,
          BreakpointRequestInfo(TestRequestId, true, false, testFileName, testLineNumber, Nil),
